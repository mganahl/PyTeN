...
class TestTimeEvolution(unittest.TestCase):        
    def setUp(self):
        ...
        self.Szexact2=np.load('Szexact.npy')
        mps=TN.FiniteMPS.random(d=self.N*[2],D=[D]*(N-1))
        #mps._D=D

        self.mpo=MPO.XXZ(Jz,Jxy,np.zeros(self.N),True)
        mps=TN.FiniteMPS.random(d=[2]*self.N,D=[D]*(N-1),dtype=complex)
        # for s in range(N-1):
        #     gate=self.mpo.get_2site_gate(s,s+1,0.01j)
        #     mps.position(s+1)
        #     mps2=mpslib.MPS(tensors=mps._tensors)
        #     mps2._position=mps.pos
        #     mps2._mat=np.copy(mps.mat)
        #     mps2.applyTwoSiteGate(gate,s,Dmax=10,thresh=1E-16)
        #     mps.apply_2site_gate(gate,s,1E-16,D=10)
        #     print(np.linalg.norm(mps2._mat-mps.mat),np.linalg.norm(mps2._tensors[s]-mps._tensors[s]),np.linalg.norm(mps2._tensors[s+1]-mps._tensors[s+1]))
        # input()

        mps.position(self.N)
        mps.position(0)

        # test=H.XXZ(Jz,Jxy,np.zeros(self.N),True)
        # for s in range(N-1):
        #     gate1=self.mpo.get_2site_gate(s,s+1,0.01j)
        #     gate2=test.twoSiteGate(s,s+1,0.01j,obc=True)
        #     print(np.linalg.norm(gate1-gate2))

        # input()
        self.timeevmpo=MPO.XXZ(Jz,Jxy,Bz*np.ones(self.N),True)                
    ...
    def testTEBD(self):
        ...
        for n in range(self.Nmax):
            ...
            plt.figure(1)
            plt.clf()
            plt.plot(sites,L,sites,self.Szexact[n,:],'x',self.Szexact2[n,:],'o')
            plt.ylim([-0.5,0.5])
            plt.draw()
            plt.show()
            plt.pause(0.01)
