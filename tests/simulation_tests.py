...
class TestFiniteDMRGXXZ(unittest.TestCase):
    ...
    def test_one_site_dmrg_float_ar(self):
        ...
        mps.position(0)
        mpo = MPO.FiniteXXZ(
            self.Jz, self.Jxy, np.zeros(self.N), dtype=mps.dtype)
        dmrg = SCT.FiniteDMRGEngine(mps, mpo, 'testXXZ')
        edmrg = dmrg.run_one_site(
            Nsweeps=2, precision=1e-10, ncv=10, verbose=1, solver='ar')
        self.assertTrue(
            (abs(edmrg - self.Eexact) / abs(self.Eexact)) < self.eps)
    ...
    def test_one_site_dmrg_float_lan(self):
        ...
        dmrg = SCT.FiniteDMRGEngine(mps, mpo, 'testXXZ')
        edmrg = dmrg.run_one_site(
            Nsweeps=2, precision=1e-10, ncv=10, verbose=1, solver='lan')
        self.assertTrue(
            (abs(edmrg - self.Eexact) / abs(self.Eexact)) < self.eps)
...
class TestMPSTrunc(unittest.TestCase):
    ...
    def test_mps_trunc_float(self):
        ...
        edmrg = dmrg.run_one_site(
            Nsweeps=2, precision=1e-10, ncv=10, verbose=1, solver='ar')
        Sz = np.diag([0.5, -0.5])
        meanSz = dmrg.mps.measure_1site_ops(
            ops=[Sz] * self.N, sites=range(self.N))
        meanSzSz = dmrg.mps.measure_1site_correlator(Sz, Sz, self.N // 2,
                                                     range(self.N))

        Dt = 32
        dmrg.mps.truncate(schmidt_thresh=1E-8, D=Dt)
        meanSztrunc = dmrg.mps.measure_1site_ops(
            ops=[Sz] * self.N, sites=range(self.N))
        meanSzSztrunc = dmrg.mps.measure_1site_correlator(
            Sz, Sz, self.N // 2, range(self.N))
...
class TestInfiniteDMRGXXZ(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.Jz = np.ones(self.N)
        self.Jxy = np.ones(self.N)
        self.Eexact = 0.25 - np.log(
            2)  #exact GS energy of XXX model in thermodynamic limit
        self.eps = 1E-4
        self.Nsweeps = 100
...
class TestITEBD(unittest.TestCase):
    ...
    def test_itebd_imag_float(self):
        dtype = np.float64
        imps = TN.MPS.random(
            D=[self.D] * (self.N + 1), d=[2] * self.N, minval=-0.5, maxval=0.5)
        impo = MPO.InfiniteXXZ(
            Jz=np.ones(len(imps)),
            Jxy=np.ones(len(imps)),
            Bz=np.zeros(len(imps)),
            dtype=imps.dtype)
        ...
        for n in range(Nmx):
            itebd.do_steps(
                dt=-dt, numsteps=numsteps, D=self.D, recanonize=None, verbose=0)
            ...
            energy += imps.measure_1site_correlator(
                Sy, Sy, 0, [1])  #the minus accounts for the missing i above
            imps.roll(1)
            energy += imps.measure_1site_correlator(Sz, Sz, 0, [1])
            energy += imps.measure_1site_correlator(Sx, Sx, 0, [1])
            energy += imps.measure_1site_correlator(
            ...
            stdout.write('\r time: %0.4f/%0.4f, energy: %.8f (exact: %0.8f)' %
                         (itebd.time, dt * Nmx * numsteps,
                          np.real(energy[0]) / 2.0, 0.25 - np.log(2)))
...
class TimeEvolutionTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        print()
        print('running time evolution test for total time T = {0}'.format(
            self.Nmax * self.dt * self.numsteps))
        ...
        Jxyar, Jzar = np.asarray(Jxyar).astype(
            np.float64), np.asarray(Jzar).astype(np.float64)
        Hsparse1 = ed.XXZSparseHam(Jxyar, Jzar, N, Nup, Z2,
                                   grid)  #get the ground state
        e, v = sp.sparse.linalg.eigsh(
            Hsparse1,
            k=1,
            which='SA',
        ...
        HB = csc_matrix((diag, (inddiag, inddiag)),
                        shape=(len(basis2), len(basis2)))
        Hsparse2 += HB
        ...
        for n in range(self.Nmax):
            ...
            stdout.write("\rED time evolution at t/T= %4.4f/%4.4f" %
                         (n * self.numsteps * np.abs(self.dt),
                          self.Nmax * self.numsteps * np.abs(self.dt)))
            stdout.flush()
            for step in range(self.numsteps):
                vnew = lan.do_step(
                    vnew,
                    self.dt,
                    zeros=np.zeros(vnew.shape, dtype=np.complex128))
                vnew /= np.linalg.norm(vnew)

        mps = TN.FiniteMPS.random(d=self.N * [2], D=[D] * (N - 1))
        self.mpo = MPO.FiniteXXZ(Jz, Jxy, np.zeros(self.N))
        mps = TN.FiniteMPS.random(
            d=[2] * self.N, D=[D] * (N - 1), dtype=complex)
        mps.position(self.N)
        mps.position(0)
        ...
        self.dmrg.mps.apply_1site_gate(
            np.array([[0.0, 0.0], [1.0, 0.0]]).view(Tensor), self.N // 2)
    ...
    def test_expvals(self):
        ...
        for s in range(self.N - 1):
            self.assertTrue(
                np.linalg.norm(gates_test[(s, s + 1)] - self.timeevmpo.
                               get_2site_gate(s, s + 1, self.dt)) < 1E-10)
...
class FiniteTEBDTests(TimeEvolutionTests):
    ...
    def testTEBD(self):
        ...
        tebd = SCT.FiniteTEBDEngine(self.dmrg.mps, self.timeevmpo,
                                    "TEBD_unittest")
        ...
        SZ = np.zeros((self.Nmax,
                       self.N))  #container for holding the measurements
        plt.ion()
        sz = [np.diag([-0.5, 0.5]) for n in range(self.N)
             ]  #a list of local operators to be measured
        ...
        for n in range(self.Nmax):
            ...
            if plot:
                ...
                plt.pause(0.01)
        print()
        print("normalized difference between TEBD and ED:{} ".format(
            np.linalg.norm(SZ - self.Szexact) / (self.Nmax * self.N)))
        self.assertTrue(
            np.linalg.norm(SZ - self.Szexact) / (self.Nmax * self.N) < 1E-5)
...
class FiniteTDVPTests(TimeEvolutionTests):
    ...
    def test_1site_FiniteTDVP(self):
        ...
        tdvp = SCT.FiniteTDVPEngine(self.dmrg.mps, self.timeevmpo,
                                    "TDVP_unittest")
        Dmax = 32  #maximum bond dimension to be used during simulation; the maximally allowed bond dimension of the mps will be
        #adapted to this value in the TimeEvolutionEngine
        thresh = 1E-16  #truncation threshold
        ...
        for n in range(self.Nmax):
            ...
            SZ[n, :] = L
            t = tdvp.run_one_site(
                dt=self.dt, numsteps=self.numsteps, krylov_dim=10, verbose=1)
            if plot:
                plt.figure(1)
                plt.clf()
