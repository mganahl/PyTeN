...
def ndarray_initializer(numpy_func, shapes, *args, **kwargs):
  ...
  if numpy_func in (np.random.random_sample, np.random.rand):
    ...
    if np.issubdtype(dtype, np.complexfloating):
      return [(std * numpy_func(*shape).view(Tensor) + mean + 1j * std *
               (numpy_func(*shape).view(Tensor) + 1j * mean)).astype(dtype)
              for shape in shapes]

    elif np.issubdtype(dtype, np.floating):
      tensors = [(std * numpy_func(*shape).view(Tensor) + mean).astype(dtype)
                 for shape in shapes]
    ...
    if np.issubdtype(dtype, np.complexfloating):
      ...
      return [
          numpy_func(shape, *args, **kwargs).view(Tensor) for shape in shapes
      ]
    ...
    if isinstance(tensors, np.ndarray):
      ...
      if shape != ():
        if not (np.prod(shape) == N):
          raise ValueError(
              'shape={0} incompatible with len(tensors)={1}'.format(shape, N))
      ...
      raise TypeError(
          'TensorNetwork.__init__(tensors): tensors has invlaid tupe {0}'.type(
              tensors))
  ...
  def __array_ufunc__(self, ufunc, method, *inputs, **kwargs):
    ...
    if out:
      ...
      for n, x in np.ndenumerate(self._tensors):
        ...
        ipts = [
            ipt[n] if isinstance(ipt, type(self)) else ipt for ipt in inputs
        ]
      ...
      result = [
          getattr(ufunc, method)(*[
              ipt[n] if isinstance(ipt, type(self)) else ipt for ipt in inputs
          ], **kwargs) for n, x in np.ndenumerate(self._tensors)
      ]
...
class MPSBase(TensorNetwork):
  ...
  def measure_1site_correlator(self, op1, op2, site1, sites2):
    ...
    if len(left_sites) > 0:
      ...
      r = ncon.ncon([A, A.conj(), op1, rs[site1]], [(-1, 1, 2), (-2, 4, 3),
                                                    (3, 2), (1, 4)])
  ...
  def prepend(self, tensors):
    """
        ...
        """
    new_tensors = tensors + [
        self.get_tensor(n).split([[self.D[n]], [self.D[n + 1]], self.shapes[n]])
...
class MPS(MPSBase):
  ...
  def roll(self, num_sites):
    ...
    new_center_matrix = ncon.ncon([self.mat, self.connector],
                                  [[-1, 1], [1, -2]])
  ...
  def get_env_right(self, site):
    ...
    if site == len(self) - 1:
      ...
      r = ncon.ncon(
          [self.centermatrix, self.centermatrix.conj()], [[-1, 1], [-2, 1]])
  ...
  def regauge(self,
    ...
    if gauge in ('left', 'l', 1):
      ...
      invy = ncon.ncon(
          [np.sqrt(inveigvals).diag(), u.conj()], [[-2, 1], [-1, 1]])
    ...
    if gauge in ('right', 'r', -1):
      ...
      invx = ncon.ncon(
          [np.sqrt(inveigvals).diag(), u.conj()], [[-1, 1], [-2, 1]])
  ...
  def canonize(self,
    ...
    if np.sum(self.D[0]) == 1:
      if np.sum(self.D[0]) != np.sum(self.D[-1]):
        raise ValueError(
            'MPS.canonize():  left and right boundary bond dimensions of the  MPS are different!'
        )
      self.position(len(self))
      self.normalize()
    else:
  ...
  def __array_ufunc__(self, ufunc, method, *inputs, **kwargs):
    ...
    if out:
      ...
      for n, x in np.ndenumerate(self):
        ...
        result.append(getattr(ufunc, method)(*ipts, **kwargs))
      matipts = [
          ipt.mat if isinstance(ipt, type(self)) else ipt for ipt in inputs
      ]
      ...
      result = [
          getattr(ufunc, method)(*[
              ipt[n] if isinstance(ipt, type(self)) else ipt for ipt in inputs
          ], **kwargs) for n, x in np.ndenumerate(self)
      ]
      ...
      for n in range(self._position, bond):
        ...
        if (n + 1) < bond:
          self[n + 1] = ncon.ncon([self.mat, self[n + 1]],
                                  [[-1, 1], [1, -2, -3]])
    if bond < self._position:
      self[self._position - 1] = ncon.ncon([self[self._position - 1], self.mat],
                                           [[-1, 1, -3], [1, -2]])
  ...
  def apply_2site_gate(self, gate, site, truncation_threshold=1E-16, D=None):
    ...
    newState = ncon.ncon(
        [self._tensors[site], self.mat, self._tensors[site + 1], gate],
  ...
  def apply_MPO(self, mpo):
    ...
    if not len(self) == len(mpo):
      raise ValueError(
          'length of mpo is different from length of mps! cannot apply mpo')
    if hasattr(mpo, 'get_tensor'):
      tensors = [
          ncon.ncon([self.get_tensor(n), mpo.get_tensor(n)],
                    [[-1, -3, 1], [-2, -4, -5, 1]]).merge([[0, 1], [2, 3],
                                                           [4]])[0]
          for n in range(len(self))
      ]
      ...
      tensors = [
          ncon.ncon([self.get_tensor(n), mpo[n]],
                    [[-1, -3, 1], [-2, -4, -5, 1]]).merge([[0, 1], [2, 3],
                                                           [4]])[0]
          for n in range(len(self))
      ]
...
class FiniteMPS(MPS):
  ...
  def __init__(self,
    ...
    if not np.sum(tensors[0].shape[0]) == 1:
      raise ValueError('FiniteMPS got a wrong shape {0} for tensor[0]'.format(
          tensors[0].shape))
  ...
  def dot(self, mps):
    ...
    O = self[0].eye(0)
    for n in range(len(self)):
      O = mf.transfer_operator([self.get_tensor(n)], [mps.get_tensor(n)], 'l',
                               O)
    return O
  ...
  def get_amplitude(self, sigmas):
    """
        ...
        """
    left = np.expand_dims(np.ones((sigmas.shape[0], self.D[0])),
                          1)  #(Nt, 1, Dl)
    for site in range(len(self)):
      tmp = ncon.ncon([
          self.get_tensor(site),
          np.eye(self.d[site])[sigmas[:, site]].astype(np.int32)
      ], [[-2, -3, 1], [-1, 1]])  #(Nt, Dl, Dr)
  ...
  def generate_samples(self, num_samples):
    ...
    lenv = np.stack([np.eye(Ds[0], dtype=dtype) for _ in range(num_samples)],
...
class FiniteMPDO(FiniteMPS):
  ...
  def __init__(self, tensors, name=None, fromview=True):
    ...
    tens = []
    for n in range(len(tensors)):
      eye = tensors[n].eye(2).merge(
          [[1, 0]])[0]  #the merge indices need to be reversed here!
  ...
  def get_mpo_tensor(self, site):
    """
        ...
        np.ndarray:    
        """
    return self.get_tensor(site).split([[self.D[site]], [self.D[site + 1]],
                                        self.shapes[site]])
  ...
  def partial_trace(self, sites):
    ...
    if set(sites) == set(range(len(self))):
      raise ValueError(
          'FiniteMPDO.partial_trace: use get_env_left or get_env_right'
    ...
    for site in range(len(self)):
      if site in sites:
        ...
        if site < (len(self) - 1):
          tensors[site + 1] = ncon.ncon([mat, tensors[site + 1]],
                                        [[-1, 1], [1, -2, -3]])
        else:
          new_tensors[-1] = ncon.ncon([new_tensors[-1], mat],
                                      [[-1, 1, -3, -4], [1, -2]])
      else:
  ...
  def get_envs_left(self, sites):
    ...
    for n in range(n1, n2 + 1):
      if n in sites:
        ls[n] = l
      l = ncon.ncon([l, self.get_tensor(n), self.eyes[n]],
                    [[1], [1, -1, 2], [2]])
    return ls
  ...
  def get_envs_right(self, sites):
    ...
    for n in range(n2, n1 - 1, -1):
      if n in sites:
        rs[n] = r
      r = ncon.ncon([r, self.get_tensor(n), self.eyes[n]],
                    [[1], [-1, 1, 2], [2]])
    return rs
  ...
  def measure_1site_ops(self, ops, sites):
    ...
    for n in range(len(sites)):
      ...
      A = self.get_tensor(sites[n])
      res.append(
          ncon.ncon([l, A, op.merge([[1, 0]])[0], r],
                    [[1], [1, 2, 3], [3], [2]]) / Z)
  ...
  def measure_1site_op(self, op, site):
    ...
    Z = self.get_env_left(len(self))
    A = self.get_tensor(site)
    res = ncon.ncon([l, A, op.merge([[1, 0]])[0], r],
                    [[1], [1, 2, 3], [3], [2]])
    return res / Z

  def transfer_op(self, site, direction, x):
    if direction in (1, 'l', 'left'):
      return ncon.ncon([x, self.get_tensor(site), self.eyes[site]],
                       [[1], [1, -1, 2], [2]])
  ...
  def measure_1site_correlator(self, op1, op2, site1, sites2):
    ...
    if len(left_sites) > 0:
      ...
      r = ncon.ncon(
          [rs[site1], self.get_tensor(site1),
           op1.merge([[1, 0]])[0]], [[1], [-1, 1, 2], [2]])
      ...
      for n in range(site1 - 1, n1 - 1, -1):
        if n in left_sites:
          ...
          A = self.get_tensor(n % N)
          op = op2.merge([[1, 0]])[0]
          res = ncon.ncon([l, A, r, op], [[1], [1, 2, 3], [2], [3]])
          c.append(res / norm)
        if n > n1:
          r = self.transfer_op(n % N, 'right', r)
      c = list(reversed(c))
    ...
    if len(right_sites) > 0:
      ...
      l = ncon.ncon(
          [ls[site1], self.get_tensor(site1),
           op1.merge([[1, 0]])[0]], [[1], [1, -1, 2], [2]])
