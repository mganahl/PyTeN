...
class MPO:
  ...
  def twoSiteGate(self, site1, site2, tau, obc=True):
    ...
    h = np.reshape(self.getTwoSiteHamiltonian(site1, site2, obc),
                   (d1 * d2, d1 * d2))
...
class BranchingHamiltonian(MPO):
  ...
  def __init__(self, J, w, chi, obc=True):
    ...
    if obc == True:
      ...
      temp[0, 0, :, :] = self._w[0] * (sig_z + tau_z) + self._chi[0] * np.kron(
          sigma_x, sigma_x)
      ...
      for n in range(1, N - 1):
        ...
        temp[7, 0, :, :] = self._w[n] * (
            sig_z + tau_z) + self._chi[n] * np.kron(sigma_x, sigma_x)
      ...
      temp[6, 0, :, :] = tau_z
      temp[7, 0, :, :] = self._w[-1] * (
          sig_z + tau_z) + self._chi[-1] * np.kron(sigma_x, sigma_x)
      mpo.append(np.copy(temp))
